...
from ultralytics.nn.modules.block import ABlock, A2C2f, C2f, C3
...
class KWConv(nn.Module):
    """
    YOLO-style Conv using KernelWarehouse
    Conv -> BN -> Act
    """

    # ===== 类级别 Warehouse Manager（所有 KWConv 共享）=====
    _warehouse_manager = None
    _layer_count = 0

    @classmethod
    def get_warehouse_manager(cls):
        if cls._warehouse_manager is None:
            cls._warehouse_manager = Warehouse_Manager(
                reduction=0.0625,
                cell_num_ratio=1.0,
                cell_inplane_ratio=1.0,
                cell_outplane_ratio=1.0,
                sharing_range=('layer', 'conv'),
                nonlocal_basis_ratio=1.0,
            )
        return cls._warehouse_manager
    ...
    def __init__(
        ...
        wm = self.get_warehouse_manager()

        # 自动生成唯一 warehouse_name（YOLO 风格）
        warehouse_name = f"kwconv_{KWConv._layer_count}"
        KWConv._layer_count += 1

        self.conv = wm.reserve(
            in_planes=c1,
            out_planes=c2,
            kernel_size=k,
            stride=s,
...
class KWBottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
        ...
        self.cv1 = KWConv(c1, c_, k[0], 1)
        self.cv2 = KWConv(c_, c2, k[1], 1, g=g)
...
class KWC3k(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, k=3):
        ...
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(KWBottleneck(c_, c_, shortcut, g, k=(k, k), e=1.0) for _ in range(n)))
...
class KWC3k2(C2f):
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        self.m = nn.ModuleList(
            KWC3k(self.c, self.c, 2, shortcut, g) if c3k else KWBottleneck(self.c, self.c, shortcut, g) for _ in range(n)
        )
...
class KWA2C2f(A2C2f):
    ...
    def __init__(self, c1, c2, n=1, a2=True, area=1, residual=False, mlp_ratio=2.0, e=0.5, g=1, shortcut=True):
        ...
        self.m = nn.ModuleList(
            nn.Sequential(*(ABlock(c_, num_heads, mlp_ratio, area) for _ in range(2))) if a2 else KWC3k(c_, c_,
                                                                                                        2, shortcut, g)
            for _ in range(n)
        )