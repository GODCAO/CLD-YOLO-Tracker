...
class GA_SPPF(nn.Module):
    def __init__(self, c1, c2, k=5):
        ...
        c_ = c1 // 2  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv_lrelu(c_ * 6, c2, 1, 1)
        self.m = nn.MaxPool2d(kernel_size=k, stride=1, padding=k // 2)
    ...
    def forward(self, x):
        ...
        gmp = F.interpolate(gmp, size=x.size()[2:], mode='bilinear', align_corners=False)

        # Concatenate all features
        cat_features = torch.cat((x, y1, y2, self.m(y2), gap, gmp), 1)

        # Final convolution
        out = self.cv2(cat_features)
...
class RepC3k2(C2f):
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(
            RepC3k(self.c, self.c, 2, shortcut, g) if c3k else RepBottleneck(self.c, self.c, shortcut, g) for _ in range(n)
        )

class RepA2C2f(A2C2f):
    ...
    def __init__(self, c1, c2, n=1, a2=True, area=1, residual=False, mlp_ratio=2.0, e=0.5, g=1, shortcut=True):
        ...
        self.m = nn.ModuleList(
            nn.Sequential(*(ABlock(c_, num_heads, mlp_ratio, area) for _ in range(2))) if a2 else RepC3k(c_, c_, 2, shortcut, g) for _ in range(n)
        )
